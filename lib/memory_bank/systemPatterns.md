...
## 2. Secure Command Dispatcher (TCP)

The primary interaction model for active commands is a secure command-response pattern over TCP.
...
## 3. Wake-on-LAN (WoL) Broadcast (UDP)

To wake a powered-off device, a different, unencrypted broadcast pattern is used.

1.  **Action:** The user taps the "Wake Up" button.
2.  **Packet Construction:** The Flutter app manually constructs a "magic packet". This is a specific byte sequence required for WoL.
    *   It starts with 6 bytes of `0xFF`.
    *   It is followed by 16 repetitions of the target device's 6-byte MAC address.
3.  **Transmission:** The magic packet is sent as a UDP datagram to the local network's broadcast address (e.g., 192.168.1.255) on port 9. This is a fire-and-forget operation; no response is expected.
4.  **Reception:** The network interface card (NIC) of the powered-off computer, if WoL is enabled in the BIOS/UEFI, receives this packet and signals the system to power on.

## 4. Per-Device Configuration Persistence
...
*   **Data Points per Device:** Each device map contains its `name`, `ip`, `mac` (for WoL), and unique `secretKey`.

## 5. UI/UX Patterns

*   **Optimistic UI:** When an action like "Unlock" or "Wake Up" is triggered, the UI immediately updates to a "sending..." state, providing instant feedback to the user before the network operation completes.
...
## 6. Server-Side Resource Management
